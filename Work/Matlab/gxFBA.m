function [gxFBAsolution,genestat,Z,gxFVAmin,gxFVAmax,Zgx,Zgmin,Zgmax] = gxFBA(model1,model2,geneList,exprVect,exprType,threshold,iter,verbFlag)
%
%USAGE
...
%   This is gxFBA version 1.2, with added support for gurobi5 solver.
%
%INPUT
...
%
%OPTIONAL OUTPUT
% genestat         structured array object with gene use status for
%                     supplied gene expression data
...
%       -Matlab R2011a, and R2012b (but gurobi5 crashes with R2013a or R2013b)
%       -Gurobi 5.0.1, 5.0.2, 5.6.0
%
%%
...
wt  = optimizeCbModel_int(model1,'max'); #Wildtype fluxes, FBA optimization.
acc = find(abs(wt.x) > eps2); #Indexes of active reactions in wild-type
lac = length(acc); #Number of active reactions in wild-type
if lac > 0
    wtc = sum( wt.x(acc) )/lac; #Average wild-type flux
else
...
if (verbFlag)
    fprintf('Calculating FVA for FBA reference state (this may take some time)...');
end
%FVA analysis at 0% of FBA optimum to find characteristic flux scale for 
%  given reference environment.
[wt_minf, wt_maxf] = fluxVariability(model1,0,'max');  #According to article, FVA is performed for condition 2. Here, FVA is performed for condition 1.
...
for i=1:length(geneID2)
    ...
    for j=1:length(d)  %each gene may lead to multiple reactions
        ...
        if ( model2.rev(d(j)) == 0 || abs(wt_maxf(d(j))-wt_minf(d(j))) < abs(wt_maxf(d(j)))+eps2 ) #Strangely, it is the reversibility under condition (1) that is considered after the OR operator.
...
%  environment, so that they will not enter the gxFBA goal function.
rem  = find( (wt_maxf > 500) & (model2.rev == 0) );  
for i=1:length(rem)
    remGX(rem(i)) = 0;
end

%remove reactions wich cannot be active in the given environment
%  determined by the FVA analysis.
remX0 = find( abs(wt_avg) < eps2);
for i=1:length(remX0)
    remGX(remX0(i)) = 0;
end
...
if (verbFlag)
    fprintf('Calculating FVA in GX-FBA optimal state (this may take some time)...');
end
...
if iter > 0
    ...
    if (verbFlag)
        fprintf('Calculating FVA in FBA (optimal) reference state to asses variability (this may take some time)... ');
    end
...
switch solver
    ...
    case 'gurobi'
        ...
        opts.Crossover      = 0;  %Keep interior point solution
    ...
    case 'gurobi5'
        ...
        params.Method         = 2; %Interior point solver
        params.Crossover      = 0; %Keep interior point solution
