function [A_bl,b_bl,c_bl,csense_bl,lb_bl,ub_bl,vartype_bl,sel_cont_sol,sel_int_sol] = ...
    omni(clp,cip,S,c,ub,sel_int,rev_int,ind_ic,b_ic,csense_ic,sel_m,b_m,wt_m,H,K,ksense,sel_int_prev)
% OMNI Creates the necessary matrices and vectors to solve a bilevel MILP with designated inner
%            and outer problem objective functions and with a set a
%            measured variables
%
...
%
% Outputs suitable for feeding to feeding into a LINDO-type MILP solver
%
% A_bl          LHS matrix 
% b_bl          RHS
% c_bl          Objective
% csense_bl     Constraint types
% lb_bl         Lower bounds
% ub_bl         Upper bounds
% vartype_bl    Variable types
%
...
Iint = sel_matrix(sel_int);
Inint = sel_matrix(~sel_int & ~sel_ic);
Iic = sparse(n_ic,n);
